...
U8 targetDeviceAddress =
    255; // 通信地址，设为0-254指定则设备地址，设为255则不指定设备(即广播),
         // 当需要使用485总线形式通信时通过该参数选中要操作的设备，若仅仅是串口1对1通信设为广播地址255即可
...
void Dbp_U8_buf(const char *sBeginInfo, const char *sEndInfo,
                const char *sFormat, const U8 *Buf, U32 Len) {
  int i;
  if (sBeginInfo) {
    Dbp("%s", sBeginInfo);
  }
...
int Cmd_PackAndTx(U8 *pDat, U8 DLen) {
  U8 buf[50 + 5 +
         CmdPacketMaxDatSizeTx] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   ...
                                   0xff}; // 发送包缓存
                                          // 开头50字节是前导码，用于唤醒可能处于睡眠状态的模块

  if ((DLen == 0) || (DLen > CmdPacketMaxDatSizeTx) ||
      (pDat == NULL)) { // 非法参数
    return -1;
  }
  ...
  buf[53 + DLen] =
      CalcSum1(&buf[51], DLen + 2); // CS 从 地址码开始算到数据体结束
...
#define cmdEnd buf[4 + cmdLen]              // 结束码
  ...
  CS +=
      byte; // 边收数据边计算校验码，校验码为地址码开始(包含地址码)到校验码之前的数据的和
  switch (RxIndex) {
  case 0: // 起始码
  ...
  case 2: // 数据体的长度
    buf[i++] = byte;
    if ((byte > CmdPacketMaxDatSizeRx) || (byte == 0)) { // 长度无效
      RxIndex = 0;
      break;
    }
    RxIndex++;
...
void Cmd_12(U8 accStill, U8 stillToZero, U8 moveToZero, U8 isCompassOn,
  ...
  buf[2] = stillToZero;
  buf[3] = moveToZero;
  buf[4] = ((barometerFilter & 3) << 1) |
           (isCompassOn & 1); // bit[2-1]: BMP280的滤波等级[取值0-3]   bit[0]:
                              // 1=已开启磁场 0=已关闭磁场
  buf[5] = reportHz;
  buf[6] = gyroFilter;
  buf[7] = accFilter;
...
void Cmd_17(U8 flag) {
  ...
  } else if (flag == 1) { // 请求采集下个数据
    Dbp("calibration request next point--\r\n");
  } else if (flag == 255) { // 请求采集最后一个数据并结束当前进行中的校准
...
/**
 * 设置关机电压和充电参数
 * @param PowerDownVoltageFlag 关机电压选择 0=3.4V(锂电池用)
 * 1=2.7V(其它干电池用)
 * @param charge_full_mV  充电截止电压 0:3962mv 1:4002mv 2:4044mv 3:4086mv
 * 4:4130mv 5:4175mv 6:4222mv 7:4270mv 8:4308mv 9:4349mv 10:4391mv
 * @param charge_full_mA 充电截止电流 0:2ma 1:5ma 2:7ma 3:10ma 4:15ma 5:20ma
 * 6:25ma 7:30ma
 * @param charge_mA      充电电流 0:20ma 1:30ma 2:40ma 3:50ma 4:60ma 5:70ma
 * 6:80ma 7:90ma 8:100ma 9:110ma 10:120ma 11:140ma 12:160ma 13:180ma 14:200ma
 * 15:220ma
 */
void Cmd_24(U8 PowerDownVoltageFlag, U8 charge_full_mV, U8 charge_full_mA,
            U8 charge_mA) {
  U8 buf[5] = {0x24};
 ...
 * 设置 空闲关机时长
 *
 * @param idleToPowerOffTime
 * 当串口没有通信且蓝牙在广播中，连续计时达到这么多个10分钟则关机  0=不关机
 */
 ...
 *
 * @param DisableBleSetNameAndCahrge 1=禁止通过蓝牙更改名称及充电参数
 * 0=允许(默认) 可能客户的产品不想让别人用蓝牙随便改，设为1即可
 */
 ...
 *
 * @param EcoTime_10s
 * 该值大于0，则开启自动节能模式(即传感器睡眠后不主动上报，或静止EcoTime_10s个10秒自动进入运动监测模式且暂停主动上报)
 * 0=不启用自动节能
 */
...
static void Cmd_RxUnpack(U8 *buf, U8 DLen) {
  ...
  case 0x10: // 模块当前的属性和状态 回复
    ...
    Dbp("\t compass: %s\r\n",
        ((buf[4] >> 0) & 0x01)
            ? "on"
            : "off"); // 字节4 bit[0]: 1=已开启磁场 0=已关闭磁场
    Dbp("\t barometer filter: %u\r\n",
        (buf[4] >> 1) &
            0x03); // 字节4 bit[1-2]:
                   // 气压计的滤波等级[取值0-3],数值越大越平稳但实时性越差
    Dbp("\t IMU: %s\r\n",
        ((buf[4] >> 3) & 0x01)
            ? "on"
            : "off"); // 字节4 bit[3]: 1=传感器已开启  0=传感器已睡眠
    Dbp("\t auto report: %s\r\n",
        ((buf[4] >> 4) & 0x01)
            ? "on"
            : "off"); // 字节4 bit[4]: 1=已开启传感器数据主动上报
                      // 0=已关闭传感器数据主动上报
    Dbp("\t FPS: %u\r\n",
    ...
    Dbp("\t Mac: %02X:%02X:%02X:%02X:%02X:%02X\r\n", buf[15], buf[16], buf[17],
        buf[18], buf[19], buf[20]);      // 字节[15-20] MAC地址
  ...
  case 0x11: // 获取订阅的功能数据 回复或主动上报
  {
    ...
    Dbp("\t subscribe tag: 0x%04X\r\n", ctl);
    Dbp("\t ms: %u\r\n",
        (U32)(((U32)buf[6] << 24) | ((U32)buf[5] << 16) | ((U32)buf[4] << 8) |
              ((U32)buf[3] << 0))); // 字节[6-3] 为模块开机后的时间戳(单位ms)

    L = 7; // 从第7字节开始根据 订阅标识tag来解析剩下的数据
    if ((ctl & 0x0001) != 0) { // 加速度xyz 去掉了重力 使用时需*scaleAccel m/s
    ...
    if ((ctl & 0x0002) != 0) { // 加速度xyz 包含了重力 使用时需*scaleAccel m/s
      tmpX = (S16)(((S16)buf[L + 1] << 8) | buf[L]) * scaleAccel;
      L += 2;
      Dbp("\tAX: %.3f\r\n", tmpX); // x加速度AX
      tmpY = (S16)(((S16)buf[L + 1] << 8) | buf[L]) * scaleAccel;
    ...
    if ((ctl & 0x0004) != 0) { // 角速度xyz 使用时需*scaleAngleSpeed rad/s
    ...
    if ((ctl & 0x0010) != 0) { // 温度 气压 高度
      ...
      tmpU32 =
          (U32)(((U32)buf[L + 2] << 16) | ((U32)buf[L + 1] << 8) | (U32)buf[L]);
      tmpU32 =
          ((tmpU32 & 0x800000) == 0x800000)
              ? (tmpU32 | 0xff000000)
    ...
    if ((ctl & 0x0200) != 0) { // 加速度xyz 去掉了重力 使用时需*scaleAccel m/s
      tmpX = (S16)(((S16)buf[L + 1] << 8) | buf[L]) * scaleAccel;
      L += 2;
      Dbp("\tasX: %.3f\r\n", tmpX); // x加速度asX
      tmpY = (S16)(((S16)buf[L + 1] << 8) | buf[L]) * scaleAccel;
  ...
  case 0x17: // 加速计高精度校准 回复
    if (buf[1] ==
        255) { // 字节1 值255
               // 表示采集完成，正在结束校准(设备需继续保持静止等待10秒钟)
      Dbp("\t calibration success, please wait 10 seconds\r\n");
    } else if (buf[1] == 254) { // 字节1 值254 表示陀螺仪自检失败
      Dbp("\t calibration fail, gyro error\r\n");
    } else if (buf[1] == 253) { // 字节1 值253 表示加速计自检失败
      Dbp("\t calibration fail, accelerometer error\r\n");
    } else if (buf[1] == 252) { // 字节1 值252 表示磁力计自检失败
      Dbp("\t calibration fail, compass error\r\n");
    } else if (buf[1] == 251) { // 字节1 值251 表示设备未在校准中
      Dbp("\t calibration fail, Hasn't started\r\n");
    } else if (buf[1] != 0) { // 值[1-250] 表示当前已采集的次数
  ...
  case 0x25: // 读取关机电压和充电参数 回复
    ...
    Dbp("\t charge_full_mV: %u\r\n",
        buf[2]); // 字节2 充电截止电压 0:3962mv 1:4002mv 2:4044mv 3:4086mv
                 // 4:4130mv 5:4175mv 6:4222mv 7:4270mv 8:4308mv 9:4349mv
                 // 10:4391mv
    Dbp("\t charge_full_mA: %u ma\r\n",
        buf[3]); // 字节3 充电截止电流 0:2ma 1:5ma 2:7ma 3:10ma 4:15ma 5:20ma
                 // 6:25ma 7:30ma
    Dbp("\t charge_mA: %u ma\r\n",
        buf[4]); // 字节3 充电电流 0:20ma 1:30ma 2:40ma 3:50ma 4:60ma 5:70ma
                 // 6:80ma 7:90ma 8:100ma 9:110ma 10:120ma 11:140ma 12:160ma
                 // 13:180ma 14:200ma 15:220ma
    break;
...
void im948_test(void) {
  ...
  {
    S8 accMatrix[9] = {// 加速计与模块标识方向一致
                       -1, 0, 0, 0, 1, 0, 0, 0, -1};
    S8 comMatrix[9] = {// 磁力计与模块标识方向一致
                       -1, 0, 0, 0, -1, 0, 0, 0, 1};
    Cmd_20(accMatrix, comMatrix);
  } break;
  case 'm': // m 设置PCB安装方向矩阵 为模块的丝印标识方向绕x轴正转90度 竖着放
  {
    S8 accMatrix[9] = {// 竖放 即加速计绕模块的x轴正转90度  ok
                       1, 0, 0, 0, 0, -1, 0, 1, 0};
    S8 comMatrix[9] = {// 竖放 即磁力计绕模块的x轴正转90度  ok
                       1, 0, 0, 0, 0, 1, 0, -1, 0};
    Cmd_20(accMatrix, comMatrix);
  } break;
  ...
  case 'r': // r 设置为关机电压2.7V  充电截止电压4.22V  充电截止电流10ma
            // 充电电流50ma  模块出厂默认就是这个配置
    Cmd_24(1, 6, 3, 3);
    break;
  case 's': // s 设置为关机电压3.4V  充电截止电压4.22V  充电截止电流15ma
            // 充电电流200ma
    Cmd_24(0, 6, 4, 14);
    break;
  ...
  case 'P': // P 设置静止节能模式的触发时长
            // EcoTime_10s大于0，则开启自动节能模式(即传感器睡眠后不主动上报，或静止EcoTime_10s个10秒自动进入运动监测模式且暂停主动上报)
            // 0=不启用自动节能
