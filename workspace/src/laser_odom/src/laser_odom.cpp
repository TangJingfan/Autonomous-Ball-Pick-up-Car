...
public:
  ICPLocalization(ros::NodeHandle &nh)
    ...
    laser_odom_pub_ = nh_.advertise<nav_msgs::Odometry>("/laser_odom", 10);
  }
...
private:
  ...
  // Store last pose for odometry calculation
  double last_pose_x_;
  double last_pose_y_;
  double last_pose_theta_;
  ...
  void scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg) {
    ...
    // Perform ICP matching to get transform
    Eigen::Matrix3d transform = performICP(scan_points);

    // Update robot pose based on ICP
    updatePose(transform);

    // Publish robot pose as PoseStamped
    publishPose();

    // Publish Odometry as laser odom
    publishLaserOdom();
  }

  void publishLaserOdom() {
    // Compute odometry (relative displacement from last pose)
    double delta_x = pose_x_ - last_pose_x_;
    double delta_y = pose_y_ - last_pose_y_;
    double delta_theta = pose_theta_ - last_pose_theta_;

    // Create Odometry message
    nav_msgs::Odometry odom_msg;
    odom_msg.header.stamp = ros::Time::now();
    odom_msg.header.frame_id = "map";
    odom_msg.child_frame_id = "base_link"; // The robot's frame
    ...
    odom_msg.twist.twist.linear.x =
        delta_x / msg->time_increment; // Approximate
    odom_msg.twist.twist.linear.y = delta_y / msg->time_increment;
    odom_msg.twist.twist.angular.z = delta_theta / msg->time_increment;

    // Publish Odometry message
    laser_odom_pub_.publish(odom_msg);

    // Store current pose as last pose for next callback
    last_pose_x_ = pose_x_;
    last_pose_y_ = pose_y_;
    last_pose_theta_ = pose_theta_;
  }
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "laser_odom_node");
  ros::NodeHandle nh;
